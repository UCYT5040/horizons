...
| Transactions | `/admin/transactions` | Unified ledger of shop purchases, event tickets, and admin adjustments — flat chronological view and a by-user fulfilment view with per-user bulk fulfill/unfulfill/refund. Users with a negative balance carry a red ⚠ badge (flat rows, by-user headers, and a Balance stat per group); local balances update as refunds are applied. CSV export of the current result set |
| Transaction Detail | `/admin/transactions/{id}` | One transaction with full buyer identity (address/phone), item/event details, and fulfill/unfulfill/refund actions. Shows the user's current balance (red warning banner when negative) and their full admin balance-adjustment history (hours, reason, date, reversed state) |
