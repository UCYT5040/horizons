...
Roles that may sign in are listed in `$lib/roles.ts` (`PRIVILEGED_ROLES` / `isPrivileged()`), which mirrors the backend `hasRole` helper — `superadmin` satisfies every check. `event_viewer` is the one role with a trimmed sidebar (Home + Events + Announcements, filtered in `Sidebar.svelte` when it is the *sole* elevated role). Every other role — `reviewer`, `fulfiller` — gets the full menu and relies on the backend to reject what it isn't allowed to reach, so some links land on 403 error states. Login always lands on `/admin/home`, which errors for any role that cannot call `/api/admin/stats`. Superadmins hand out roles at `/admin/manage-admins`; `superadmin` itself is never assignable through the UI.
...
| Transactions | `/admin/transactions` | Transaction ledger — flat and by-user views, kind/fulfilment/refund filters, scoped search, inline fulfil/unfulfil/refund plus per-user bulk actions. The id in each row links to the detail page. Admin, Superadmin, and Fulfiller — a fulfiller-only account lands here after login |
| Transaction Detail | `/admin/transactions/{id}` | Everything about one purchase: item + variant + shop + cost charged vs current list price, purchase date, and the buyer's name/email/Slack/current balance/shipping address (called out explicitly when no address is on file). Fulfil and unfulfil from here; refund is admin-only and hidden from fulfillers. Free-text admin note per transaction (`Transaction.adminNote`, 1000 chars, empty clears). Admin, Superadmin, and Fulfiller |
...
- **Second review queue** (a.k.a. "send to admin"): Reviewers can escalate a submission via the "Second Review" verdict option (required note). Escalated submissions leave the pending queue and appear in an amber admin-only gallery section; only admins can approve/reject them or return them to the reviewer queue (the escalation shows in the review timeline)
- **Detail view**: Multi-panel layout showing:
  ...
  - Action bar (approve / request changes / second review)
